...
class MongoDatabase {
  ...
  static Future<List<Map<String, dynamic>>> getItemsFromMongoDB() async {
    try {
      ...
      if (e is SocketException && _retryCallback != null) {
        _retryCallback!(Order(quantities: {}, totalAmount: 0, orderDate: DateTime.now(), token: '', email: ''));
      }
      return [];
    }
  }
        ...
        if (currentStock != null && currentStock is int) {
          if (currentStock >= quantity + lockedQuantity) {
            await collection.update(
              where.eq('itemName', itemName),
              modify.set('locked', true).set('lockedQuantity', lockedQuantity + quantity),
            );
  ...
  static Future<bool> unlockStock(String itemName, int quantity) async {
    try {
      var db = await Db.create(MONGO_URL);
      await db.open();
      var collection = db.collection('feed');
      ...
      await collection.update(
        where.eq('itemName', itemName),
        modify.set('locked', false).set('lockedQuantity', 0),
      );

      _lockTimestamps.remove(itemName);
